// ── view.js ───────────────────────────────────────────────────────────────
// View layer. All DOM mutations live here; no fetch/state writes.
...
// ── Year menu ──────────────────────────────────────────────────────────────
export function renderYearMenu(selectedYears) {
  const menu = dom.yearMenu();
  menu.innerHTML = '';
    ...
    li.innerHTML = `
      <label class="dropdown-item ${active ? 'active-item' : ''}">
        <input type="checkbox" ${active ? 'checked' : ''}
          style="accent-color:var(--accent);width:15px;height:15px;" value="${y}" />
        ${y}
      </label>`;
    li.dataset.year = y;
    menu.appendChild(li);
...
export function renderMALModal(a, modal) {
  ...
  dom.modalBody().innerHTML = _detailHTML({
    ...
    tags:       [],
    desc:       a.synopsis || 'No description available.',
...
function _detailHTML({ banner, cover, title, native, score, type, status,
    episodes, year, studios, genres, tags, desc, anilistUrl, malUrl }) {
...
export function renderPills(state, onRemove) {
  ...
  state.years.forEach(y    => add(y,              () => onRemove('year',    y)));
}
